...
function periodGuide(period) {
  if (period === "1年未満") {
    return "保有期間が短期（1年未満）のため、直近の四半期決算・月次データ・進捗率・直近の開示など、短期で変化が見える指標を中心に照合し、watch_pointsも高頻度（月次・四半期）で確認できるものにしてください。";
  }
  if (period === "1〜5年") {
    return "保有期間が中期（1〜5年）のため、通期業績の推移・中期経営計画の進捗・事業セグメントの成長性を中心に照合してください。";
  }
  if (period === "5年以上") {
    return "保有期間が長期（5年以上）のため、四半期の細かい増減より、事業の競争優位・市場の構造変化・財務の健全性・長期トレンドを中心に照合し、watch_pointsも半期〜年次で確認できる大きな変化を中心にしてください。";
  }
  return "";
}
...
${pg ? pg + "\n" : ""}
検索の方針：検索は2回まで（1回目は「銘柄名 最新決算」で広く取得し、足りない場合のみ理由に直結するキーワードで1回追加）。十分な情報が得られたら追加検索はしない。
...
検索の方針：検索は2回まで。十分な情報が得られたら追加検索はしない。
...
const WEB_SEARCH_TOOL = {
  ...
  name: "web_search",
  // プロンプトで「検索は2回まで」と指示しつつ、安全弁として4回を上限にする。
  // 上限を厳しくしすぎると検索が途中で打ち切られ精度が落ちるため、指示＋緩い上限の二段構え。
  max_uses: 4,
};
